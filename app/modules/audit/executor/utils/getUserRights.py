...
def get_user_right_local(subcategory):
    ...
    subprocess.run(
        "secedit /export /cfg %temp%\\secpol.cfg /areas user_rights",
        shell=True,
        check=True,
    )
...
def get_user_rights_actual_value(win_client: Client, audit: Audit):
    try:
        ...
        actual_value_list = output.split("====")
    ...
    if "(" in expected_value or ")" in expected_value:
        if right_type == "SeIncreaseBasePriorityPrivilege":
            sid_set_list.append(
                set(
                    [
                        user_right_dict["administrators"],
            ...
            sid_set_list.append(
                set(
                    [
                        user_right_dict["administrators"],
                        user_right_dict["virtual machines"],
                    ]
                )
            )
            ...
            sid_set_list.append(
                set(
                    [
                        user_right_dict["administrators"],
                        user_right_dict["nt service\wdiservicehost"],
                    ]
                )
            )
...
def compare_user_rights(audit: Audit, actual_value_list, data_dict):
    # user rights
    ...
    right_type_values = audit.rule.right_type
    ...
    right_type = str(right_type_values)
    expected_value = str(value_data_values)
    actual_value = actual_value_list

    try:
        result = compare_user_right_result(right_type, expected_value, actual_value)
        if result:
            pass_result = True
        else:
            pass_result = False
